...
export const findConnectionById = (id: string): Connection | null => (
  registry.get(id)
);
...
export const registerConnection = (socket: Socket): Connection => {
  ...
  socket.once('disconnecting', () => {
    ...
    connection.channels.forEach((channel) => announceChannel(channel, {
  ...
  socket.on(ClientCommandType.JOIN, ({ channel }) => joinChannel(
    connection,
    channel,
  ));
  socket.on(ClientCommandType.PART, ({ channel }) => partChannel(
    connection,
    channel,
  ));
  socket.on(ClientCommandType.MESSAGE, ({ channel, message }) => sendMessage(
    connection,
    channel,
    message,
  ));
